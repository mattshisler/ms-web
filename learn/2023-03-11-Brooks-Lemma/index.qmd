...
recall the principle property of a Markov Chain, the *Markov Property*. The transition probabilities for $X_t$ depend only on the previous state $X_{t-1}$. One way to describe the Markov property in the context of a time-ordered evolving process is to say that, conditioned on the present, the past and future are independent. 
$$
