...
We might desire to sample full conditionals for $\boldsymbol\phi_i$ rather than the full joint distribution for $\boldsymbol\phi'$. In this appoach the derivaations are as follows. . .
...
## FC for Covariance of the Spatial Random Vector
...
&= \mathbf{D}\boldsymbol\Phi^T\boldsymbol\Phi - \rho\sum_{k=1}^nw_{k+}\boldsymbol\phi_k\sum_{\substack{j \in \mathcal{N}_k \\ j \, > \, k}}\frac{w_{kj}}{w_{k+}}\boldsymbol\phi_j^T\mathbf{I}_p \quad \quad \text{set } \mathbf{B}_{kj} = \frac{w_{kj}}{w_{k+}}\mathbf{I}_p\\
...
::: {.callout-note}
This is suspiciously close to $(\mathbf{D}-\rho\mathbf{W})\boldsymbol\Phi^T\boldsymbol\Phi$. My *guess* is that $2\boldsymbol\Phi^T\mathbf{W}_U\boldsymbol\Phi = \boldsymbol\Phi^T\mathbf{W}\boldsymbol\Phi = \mathbf{W}\boldsymbol\Phi^T\boldsymbol\Phi$, but I will need to verify this.
:::
