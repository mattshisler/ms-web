...
$$
p(\boldsymbol\phi_k \,|\, \{\boldsymbol\phi_{j,0}\}_{j=1}^{k-1},\{\boldsymbol\phi_{i}\}_{i={k+1}}^n) = \exp\left\{-\frac{1}{2w_{k+}}\left(\boldsymbol\phi_k - \rho\sum_{\substack{j\in\mathcal{N}_k \\ j \in P_k}}\mathbf{B}_{kj}\boldsymbol\phi_j\right)^T\boldsymbol\Lambda^{-1}\left(\boldsymbol\phi_k - \rho\sum_{\substack{j\in\mathcal{N}_k \\ j \in P_k}}\mathbf{B}_{kj}\boldsymbol\phi_j\right)\right\}
$$

As it so happens, the $k$th term also reduces nicely, following ths same derivation as for the 1st term.
$$
\begin{align*}
\frac{p(\boldsymbol\phi_k \,|\, \{\boldsymbol\phi_{j,0}\}_{j=1}^{k-1},\{\boldsymbol\phi_{i}\}_{i={k+1}}^n)}{p(\boldsymbol\phi_{k,0} \,|\, \{\boldsymbol\phi_{j,0}\}_{j=1}^{k-1},\{\boldsymbol\phi_{i}\}_{i={k+1}}^n)} 
&=
\exp\left\{-\frac{1}{2w_{k+}}\left[\left(\boldsymbol\phi_k\right)^T\boldsymbol\Lambda^{-1}\left(\boldsymbol\phi_k\right) \right]\right\}
\end{align*}
$$

Earlier we dropped the denominator in the $n$th term of Brook's factorization, but it is actually easier to include it to simplify the $n$th term. Re-check this derivation. . . it feels. . . wrong.
