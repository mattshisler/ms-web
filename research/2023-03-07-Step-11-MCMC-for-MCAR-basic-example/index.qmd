...
&\propto \big|(\mathbf{D} - \rho\mathbf{W})^{-1} \otimes \Lambda\big|^{-1/2} \exp\left\{-\frac{1}{2}\left(\boldsymbol\phi'\right)^T\left((\mathbf{D} - \rho\mathbf{W})^{-1}\otimes\boldsymbol\Lambda\right)^{-1}\boldsymbol\phi'\right\}\\
&\quad \; \times \big|\boldsymbol\Lambda\big|^{-(\nu + p + 1)/2} \exp\left\{-\frac{1}{2}\text{Tr}(\boldsymbol\Lambda^{-1}\mathbf{R})\right\}\\
&\propto \big|\boldsymbol\Lambda\big|^{-p/2} \exp\left\{-\frac{1}{2}\text{Tr}\left(\left((\mathbf{D} - \rho\mathbf{W})\otimes\boldsymbol\Lambda^{-1}\right)\boldsymbol\phi'\left(\boldsymbol\phi'\right)^T\right)\right\}\\
&\quad \; \times \big|\boldsymbol\Lambda\big|^{-(\nu + p + 1)/2} \exp\left\{-\frac{1}{2}\text{Tr}(\boldsymbol\Lambda^{-1}\mathbf{R})\right\}
\end{align*}
$$
At this step I need to somehow factor $\boldsymbol\Lambda^{-1}$ from the trace in the first density. There is an identity which would be useful, namely that for square matrices $\mathbf{A}$ and $\mathbf{B}$, $\text{Tr}(\mathbf{A} \otimes \mathbf{B}) = \text{Tr}(\mathbf{A})\text{Tr}(\mathbf{B})$, but I am not sure how to apply that here with the $\boldsymbol\phi'$ terms also present in the trace.
...
\end{align*}
$$
where the last line follows by application of Brook's Lemma. Choose $\boldsymbol\phi'_0 = \boldsymbol{0}$ for the fixed point in the support of $\boldsymbol\phi'$. This should simplify things later.
...
We have a particularly tedious product of ratios in the last line of the above derivation. Let's focus on the first term, then the $k$th term, to understand the form of the full conditional for $\boldsymbol\Lambda$.
...
&= 
\frac{\exp\left\{-\frac{1}{2w_{1+}}\left(\boldsymbol\phi_1-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right)^T\boldsymbol\Lambda^{-1}\left(\boldsymbol\phi_1-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right)\right\}}{\exp\left\{-\frac{1}{2w_{1+}}\left(\boldsymbol{0}-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right)^T\boldsymbol\Lambda^{-1}\left(\boldsymbol{0}-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right)\right\}}\\
&=
\frac{\exp\left\{-\frac{1}{2w_{1+}}\left(\boldsymbol\phi_1-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right)^T\boldsymbol\Lambda^{-1}\left(\boldsymbol\phi_1-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right)\right\}}{\exp\left\{-\frac{1}{2w_{1+}}\left(-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right)^T\boldsymbol\Lambda^{-1}\left(-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right)\right\}}\\
&=
\exp\left\{-\frac{1}{2w_{1+}}\left(\boldsymbol\phi_1-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right)^T\boldsymbol\Lambda^{-1}\left(\boldsymbol\phi_1-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right) + \frac{1}{2w_{1+}}\left(-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right)^T\boldsymbol\Lambda^{-1}\left(-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right)\right\}\\
&=
\exp\left\{-\frac{1}{2w_{1+}}\left[\left(\boldsymbol\phi_1-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right)^T\boldsymbol\Lambda^{-1}\left(\boldsymbol\phi_1-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right) -\left(-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right)^T\boldsymbol\Lambda^{-1}\left(-\rho\sum_{j\in\mathcal{N}_i}\mathbf{B}_{1j}\boldsymbol\phi_j\right)\right]\right\}\\
&=
\exp\left\{-\frac{1}{2w_{1+}}\left[\left(\boldsymbol\phi_1\right)^T\boldsymbol\Lambda^{-1}\left(\boldsymbol\phi_1\right) \right]\right\}
\end{align*}
$$
