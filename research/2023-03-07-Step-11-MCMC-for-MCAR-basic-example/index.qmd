...
Is this actually going to work? I still have to consider $p(\boldsymbol\phi' = \mathbf{0} \,|\, \boldsymbol\Lambda,\rho)$ which is a function of $\boldsymbol\Lambda$ 
...
Let $Q_k$ represents the set of locations for which $\boldsymbol\phi_j$ is set to $\boldsymbol{0}$ and $P_k$ be the set of locations for which we retain $\boldsymbol\phi_i$ as a variate at term $k$ in Brook's factorization. 


Then the conditional for $\boldsymbol\phi_k$ will have the form.
