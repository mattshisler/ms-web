...
\end{align*}
$$
The effect is that the Kronecker product in the covariance is commuted, hence the name *commutation* matrix for $\mathbf{K}^{(p,n)}$. Then when the resulting Kronecker product is expanded, the scalar elements of the common non-spatial covariance matrix $\Lambda$ are multiplied with identical blocks $(\mathbf{D}-\rho\mathbf{W})$. It is in this form that we can introduce different $\rho_k$ for each of the $p$ elements. 
...
It is actually more convenient to work with the precision matrix, $\left[\boldsymbol\Lambda \otimes(\mathbf{D}-\rho\mathbf{W})^{-1}\right]^{-1} = \boldsymbol\Lambda^{-1} \otimes(\mathbf{D}-\rho\mathbf{W})$
...
where $\lambda^{(-1)}_{ij}$ are the scalar elements of $\boldsymbol\Lambda^{-1}$.
