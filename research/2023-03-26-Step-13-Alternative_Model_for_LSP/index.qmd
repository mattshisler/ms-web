...
\end{align*}
$$
where $\mathbf{B}$ is a matrix with rows eaual to $\boldsymbol\beta^T_s$ (different than how it was defined $\mathbf{B}$ in previous posts) and $\mathbf{D}$ is a diagonal matrix of the row sums from $\mathbf{W}$. 

$$
\begin{align*}
\sigma^2 \,|\, \text{rest} &\sim 
\end{align*}
$$
