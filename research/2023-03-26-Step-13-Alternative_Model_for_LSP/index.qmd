...
ggplot(spat_domain) +
  geom_tile(aes(x, y, fill=beta_mat[1,])) +
  scale_y_reverse() +
  scale_fill_gradientn(colors = viridis(10)) +
  coord_fixed() + 
  theme_void()
...
ggplot(spat_domain) +
  geom_tile(aes(x, y, fill=beta_mat[2,])) +
  scale_y_reverse() +
  scale_fill_gradientn(colors = viridis(10)) +
  coord_fixed() + 
  theme_void()
...
Plot first and second elements of $\boldsymbol\delta_{s,t}$ by year.
```{r}
#| code-fold: true
...
<!-- Scratch code: Test some results for the beta full conditional calculation. - computing V. -->
...
<!-- Scratch code: Test some more results for the beta full conditional calc - computing M. -->
...
<!-- Scratch code: Test some more results for the Lambda full conditional calc - computing H. -->
...
for (iter in 2:niters){
  ...
  for (s in 1:n){
    ...
    temp <- 0
    for (j in 1:k){
      temp <- temp + t(kronecker(t(Z[j,,s]),diag(p)))%*%inv_Omega_arr[,,j]%*%(mc_rho_eps*resid[,j,neighbor_idx[[s]]]%*%neighbor_weights +w_plus[s]*delta[,j,s])
...
Sample a handful of locations and constuct traceplots of their beta vectors.
```{r}
#| code-fold: true
...
beta_subscripts <- c("0,1", "0,2", "1,1", "1,2")
...
Construct traceplots of the Lambda matrix (upper triangle).
```{r}
#| code-fold: true
par(mfrow = c(2,2))
...
ggplot(spat_domain) +
  geom_tile(aes(x, y, fill=hat_beta_mat[4,])) +
  scale_y_reverse() +
  scale_fill_gradientn(colors = viridis(10)) +
  coord_fixed() + 
  theme_void()
...
ggplot(spat_domain) +
  geom_tile(aes(x, y, fill=hat_beta_mat[4,]-beta_mat[4,])) +
  scale_y_reverse() +
  scale_fill_gradientn(colors = viridis(10), limits = c(min(beta_mat[4,]),max(beta_mat[4,]))) +
  coord_fixed() + 
  theme_void()
