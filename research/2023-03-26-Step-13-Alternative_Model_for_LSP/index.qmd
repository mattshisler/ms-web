...
Omega <- matrix(c(1, 0.5,
                   0.5, 1), byrow = T, ncol = p)
...
Lambda <- matrix(c(  1, 0.8,   0,   0,
                   0.8,   1,   0,   0,
                     0,   0,   1, 0.6,
                     0,   0, 0.6,   1 ), byrow = T, ncol = p*q)
...
spat_domain <- cbind(spat_domain_temp, t(delta[1,,]), t(delta[2,,]))
colnames(spat_domain)[4:23] <- c(paste0("delta_1_s_",seq(1:10)),paste0("delta_2_s_",seq(1:10)))
...
We'll begin with $\rho_\epsilon$ and $\rho_beta$ fixed. This way we can focus
